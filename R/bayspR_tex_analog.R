...
#' @return AnLocs=- Centroids of the grid cells selected as analogs, as (lon,lat) pairs
...
bayspR_tex_analog<-function(gdgt,prior_mean,prior_std,search_tol,runname,varargin=1000,data="raw",complete=TRUE){
  ...
  Preds<-post_mean+matrix(stats::rnorm(Nd*dim(alpha_samples)[2],0,1),nrow = Nd,ncol=dim(alpha_samples)[2])*post_sig
