#' MBT'6Me index
#'
#' Calculates the MBT'6Me index (De Jonge et al., 2014) for brGDGTs presented as either peak areas or fractional abundances.
...
#' @return The function returns the MBT'6Me value for each sample provided.
...
MBT6<-function(gdgt,complete=TRUE,na.ignore=FALSE){
  if(na.ignore==TRUE){gdgt[is.na(gdgt)]<-0}
  formula<-"( gdgt$Ia + gdgt$Ib + gdgt$Ic )/( gdgt$Ia + gdgt$Ib + gdgt$Ic + gdgt$IIa6 + gdgt$IIb6 + gdgt$IIIa6 )"
