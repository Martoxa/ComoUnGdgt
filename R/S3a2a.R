#' Ratio of IIIa to IIa
#'
#' Calculates the ratio proposed by Xiao et al., (2016) of IIIa to IIa for brGDGTs presented as either peak areas or fractional abundances.
...
#' @return The function returns the SIIIa/SIIa value for each sample provided.
#' @references Xiao, W., Wang, Y., Zhou, S., Hu, L., Yang, H., & Xu, Y. (2016). Ubiquitous production of branched glycerol dialkyl glycerol tetraethers (brGDGTs) in global marine environments: a new source indicator for brGDGTs. Biogeosciences, 13(20), 5883-5894.reference
#' @export

S3a2a<-function(gdgt,complete=TRUE,na.ignore=FALSE){
  if(na.ignore==TRUE){gdgt[is.na(gdgt)]<-0}
  formula<-"( gdgt$IIIa5 + gdgt$IIIa6 )/( gdgt$IIa5 + gdgt$IIa6 )"
