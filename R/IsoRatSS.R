#' Isomer 6 + 7Me Ratio
#'
#' Calculates the IR6+7Me ratio (Wang et al., 2021) for brGDGTs presented as either peak areas or fractional abundances.
...
#' @return The function returns the IR6+7Me value for each sample provided.
...
IsoRatSS<-function(gdgt,complete=TRUE,na.ignore=FALSE){
  if(na.ignore==TRUE){gdgt[is.na(gdgt)]<-0}
  formula<-"((( gdgt$IIa6 + gdgt$IIb6 + gdgt$IIc6 + gdgt$IIIa6 + gdgt$IIIb6 + gdgt$IIIc6 )/( gdgt$IIa5 + gdgt$IIb5 + gdgt$IIc5 + gdgt$IIIa5 + gdgt$IIIb5 + gdgt$IIIc5 + gdgt$IIa6 + gdgt$IIb6 + gdgt$IIc6 + gdgt$IIIa6 + gdgt$IIIb6 + gdgt$IIIc6 ))/(( gdgt$IIa7 + gdgt$IIIa7 )/( gdgt$IIa5 + gdgt$IIIa5 + gdgt$IIa6 + gdgt$IIIa6 + gdgt$IIa7 + gdgt$IIIa7 )))/2"
