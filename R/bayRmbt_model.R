#' BAYMBT calibration model
#'
#' Generates the BAYMBT calibration model for MBT5Me measured in soils and peats.
...
#' @return Generates parameter files for the input type of sample and model. Files are saved as RDS files.
...
bayrmbt_model<-function(Tmodel,Type){
  ...
  if(Type == "soil"){
    calibs<-utils::read.csv(file("soil_calibration_data.csv"),head=TRUE,sep=',')
  } else if(Type == "lake"){
    calibs<-utils::read.csv(file("lake_calibration_data.csv"),head=TRUE,sep=',')
  ...
  for (i in 1:Nc) {
    ...
    tau2_val<-as.numeric(tau2hat*10*stats::runif(1))
...
##discard first 200 samples of each chain as burn-in
  ...
  Ws_var<-mean(apply(b_draws[,,1],1,stats::var))
...
##Plot posteriors with priors
  ...
  posts<-ks::kde(c(slope),eval.points = xts) #x<-post$eval, y<-post$estimate
  priors<-stats::dnorm(xts,mean = m_p[1],sd=sqrt(mean(cov_p[1,])))

  #Intercept
  intercept<-b_draws[,,2]
  xti<-seq(min(intercept)-0.01,max(intercept)+0.01,by=0.0005)
  posti<-ks::kde(c(intercept),eval.points = xti)
  priori<-stats::dnorm(xti,mean = m_p[1],sd=sqrt(mean(cov_p[1,])))

  #Error variance
  xte<-seq(0,0.02,by=0.0001)
  poste<-ks::kde(c(tau2_draws),eval.points = xte)
  priore<-b^a/gamma(a)*xte^(-a-1)*exp(-b/xte)

  #Plot
  graphics::par(mfrow=c(1,3))
  plot(xts,posts$estimate,type='l',col="red",lwd=2, main = "Slope",ylab = "Prob. density",xlab="")
  graphics::lines(xts,priors,col="black",lwd=2)
  graphics::legend("topleft",legend = c("Prior","Posterior"),col = c("black","red"),lty=1,lwd = 2)

  plot(xti,posti$estimate,type='l',col="red",lwd=2, main = "Intercept",ylab="",xlab="")
  graphics::lines(xti,priori,col="black",lwd=2)
  graphics::legend("topleft",legend = c("Prior","Posterior"),col = c("black","red"),lty=1,lwd = 2)

  plot(xte,poste$estimate,type='l',col="red",lwd=2, main = "Error variance (t^2)",ylab="",xlab="")
  graphics::lines(xte,priore,col="black",lwd=2)
  graphics::legend("topleft",legend = c("Prior","Posterior"),col = c("black","red"),lty=1,lwd = 2)
