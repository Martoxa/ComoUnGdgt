...
bayrmbt_predict<-function(gdgt,prior_mean,prior_std,Tmodel,Type,data="raw",complete=TRUE,na.ignore=FALSE){
  ...
  output.ens<-post_mean+matrix(stats::rnorm(nd*n_draws),nrow = nd,ncol = n_draws)*post_sig
  ...
  output.T<-t(apply(output.ens,1,FUN = stats::quantile,na.rm=TRUE,probs=c(0.025,0.5,0.975)))
