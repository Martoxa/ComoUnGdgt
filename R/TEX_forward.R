...
TEX_forward<-function(lat,lon,t,type="SST",mode="standard",tolerance=NA){
  ...
  if(mode=="standard"){
    ...
    tex86<-matrix(stats::rnorm(n=dim(alpha_samples)[1]*dim(alpha_samples)[2],mean=c(t*beta_samples+alpha_samples),sd=c(kronecker(matrix(1,length(t),1),sqrt(Params$tau2.samples)))),nrow=dim(alpha_samples)[1],ncol = dim(alpha_samples)[2])
