...
bayspR_tex<-function(gdgt,lon,lat,prior_std,runname,varargin=1000,data="raw",complete=TRUE,na.ignore=FALSE){
  ...
  num_below_dist<-which(sorted_dists_prior == utils::tail(c(sorted_dists_prior)[c(sorted_dists_prior)<max_dist],n=1),arr.ind = FALSE)
  ...
  Preds<-post_mean+matrix(stats::rnorm(Nd*Nsamps,0,1),nrow = Nd,ncol=Nsamps)*post_sig

  #Take the percentiles
  Output_Preds<- if(length(dats)==1){stats::quantile(sort(Preds),probs=c(0.05,0.5,0.95))} else if(length(dats)>1){t(apply(apply(Preds,1,sort),2,stats::quantile,probs=c(0.05,0.5,0.95)))}
