#' Degree of cyclisation of tetramethylated brGDGTs
#'
#' Calculates the #ringstetra index (Sinninghe Damsté, 2016) for brGDGTs presented as either peak areas or fractional abundances.
...
#' @return The function returns the #ringstetra value for each sample provided.
...
RinTet<-function(gdgt,complete=TRUE,out="all",na.ignore=FALSE){
  if(na.ignore==TRUE){gdgt[is.na(gdgt)]<-0}
  formula<-"( gdgt$Ib + 2*( gdgt$Ic ))/( gdgt$Ia + gdgt$Ib + gdgt$Ic )"
